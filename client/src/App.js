...
function App() {
  ...
  const connectSocket = () => {
    ...
    socket.current.on('userDataCreated', (newUserData) => {
      if (!isAllView && newUserData.user === user._id) {
        setUserDataList((prev) => [...prev, newUserData]);
      } else if (isAllView && user.role === 'admin') {
        setUserDataList((prev) => [...prev, newUserData]);
        if (newUserData.user !== user._id) {
          toast.info(`New user data added by ${newUserData.user?.username || 'user'}`, {
            position: 'top-right',
            autoClose: 3000,
          });
        }
    ...
    socket.current.on('userDataUpdated', (updatedUserData) => {
      setUserDataList((prev) =>
        prev.map((data) => (data._id === updatedUserData._id ? updatedUserData : data))
      );
      if (isAllView && user.role === 'admin' && updatedUserData.user !== user._id) {
        toast.info(`User data updated by ${updatedUserData.user?.username || 'user'}`, {
          position: 'top-right',
          autoClose: 3000,
        });
      }
    ...
    socket.current.on('userDataDeleted', (deletedId) => {
      setUserDataList((prev) => prev.filter((data) => data._id !== deletedId));
  ...
  const fetchUserData = async () => {
    try {
      const url = `${API_URL}/userdata${isAllView ? '?all=true' : ''}`;
  ...
  const addUserData = async (formData) => {
    try {
      const res = await apiCall(`${API_URL}/userdata`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formData),
      });
      if (res.ok) {
        toast.success('User data added successfully!', { position: 'top-right', autoClose: 3000 });
        setIsModalOpen(false);
      } else {
        const data = await res.json();
        toast.error(data.message || 'Failed to add user data.', {
          position: 'top-right',
          autoClose: 3000,
        });
      }
      ...
      toast.error('Error adding user data.', { position: 'top-right', autoClose: 3000 });
    }
  };

  const updateUserData = async (id, formData) => {
    try {
      const res = await apiCall(`${API_URL}/userdata/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formData),
      });
      if (res.ok) {
        toast.success('User data updated successfully!', { position: 'top-right', autoClose: 3000 });
        setIsModalOpen(false);
        setEditingData(null);
      } else {
        const data = await res.json();
        toast.error(data.message || 'Failed to update user data.', {
          position: 'top-right',
          autoClose: 3000,
        });
      }
      ...
      toast.error('Error updating user data.', { position: 'top-right', autoClose: 3000 });
    }
  };

  const deleteUserData = async (id) => {
    try {
      const res = await apiCall(`${API_URL}/userdata/${id}`, { method: 'DELETE' });
      if (res.ok) {
        toast.success('User data deleted successfully!', { position: 'top-right', autoClose: 3000 });
      } else {
        const data = await res.json();
        toast.error(data.message || 'Failed to delete user data.', {
          position: 'top-right',
          autoClose: 3000,
        });
      }
      ...
      toast.error('Error deleting user data.', { position: 'top-right', autoClose: 3000 });
    }
  };
  ...
  const toggleView = () => {
    ...
    toast.info(`Switched to ${!isAllView ? 'All User Data' : 'My User Data'} view`, {
      position: 'top-right',
      autoClose: 3000,
    });
  };
  ...
  const handleSubmit = (formData) => {
    if (editingData) {
      updateUserData(editingData._id, formData);
    } else {
          ...
          >
            ...
            {user.role === 'admin' && (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
              >
                {isAllView ? 'View My User Data' : 'View All User Data'}
              </motion.button>
            )}
            <motion.button
              onClick={() => openModal()}
            ...
            >
              <UserDataForm onSubmit={handleSubmit} initialData={editingData} onCancel={closeModal} />
            </Modal>
          </motion.div>
        ) : (
          <motion.div
